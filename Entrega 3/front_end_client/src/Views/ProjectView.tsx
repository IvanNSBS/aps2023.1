...
const ProjectView: FC<ProjectsProps> = (props: ProjectsProps): ReactElement => {
    ...
    useState(() => {
        ...
        const fetchData = async function(){
            const allDocs = await projectsPresenter.getAllProjectDocuments(projectInfo);
            if(!allDocs)
                return;
    ...
    const create_document = async function() {
        ...
        const newDoc = await projectsPresenter.addDocumentToProject(projectInfo, document_name);
        if(!newDoc)
            return;
    ...
    const changeProjectName = async function(evt: any){
        ...
        projectsPresenter.changeProjectName(session, project.id, value);
    }
